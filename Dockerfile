...
RUN apk add --update --no-cache --virtual build-dependencies git ca-certificates && \
    git clone --depth 1 -b 0.14.2 https://github.com/Chia-Network/chia-exporter.git && \
    cd chia-exporter && \
...
RUN apt-get install git python3.11-venv python3-virtualenv lsb-release sudo procps tmux net-tools vim iputils-ping netcat-traditional -y
